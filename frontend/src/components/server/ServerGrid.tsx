  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {[...Array(8)].map((_, idx) => (
          <div key={idx} className="bg-white dark:bg-slate-900 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-slate-800 animate-pulse">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="h-6 bg-gray-200 dark:bg-slate-800 rounded-full w-12"></div>
            </div>
          </div>
        ))}
      </div>
  ...
  if (servers.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-gray-500 dark:text-slate-400">Try adjusting filters or searching for other keywords</p>
      </div>
    );
  }
