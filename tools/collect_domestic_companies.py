...
class DomesticCompanyCollector:
    ...
    def __init__(self):
        ...
        # 百度相关关键词
        self.baidu_keywords = ["baidu", "百度", "ernie", "文心", "qianfan"]
        # 华为相关关键词
        ...
        # 美团相关关键词
        self.meituan_keywords = ["meituan", "美团", "dianping", "ele-me", "mtwm"]
        # 小米相关关键词
        self.xiaomi_keywords = ["xiaomi", "小米", "miot", "mijia", "xiaomiiot"]
        # 网易相关关键词
        self.netease_keywords = ["netease", "网易", "youdao", "music163", "cloudmusic"]
    ...
    def search_github_repos(self, keywords: List[str], org: Optional[str] = None) -> List[Dict]:
        ...
        for keyword in keywords:
            try:
                if org:
                    url = (
                        f"{self.github_api}/search/repositories?q={keyword}+user:{org}&per_page=10"
                    )
                else:
    ...
    def _infer_categories(self, repo: Dict) -> List[str]:
        ...
        # AI/ML
        if any(
            k in f"{name} {desc} {topics}"
            for k in ["ai", "llm", "gpt", "nlp", "ernie", "qianfan", "modelarts"]
        ):
