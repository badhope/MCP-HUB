...
def calculate_completeness_score(server: Dict) -> Dict[str, Any]:
    ...
    if categories and len(categories) > 0:
        ...
        score_details["factors"].append({
            "name": "categories",
            "value": 10,
            "reason": f"有 {len(categories)} 个分类标签"
        })
    else:
    ...
    if topic_count >= 5:
        ...
        score_details["factors"].append({
            "name": "topics",
            "value": 10,
            "reason": f"有 {topic_count} 个 topics，功能丰富"
        })
    elif topic_count >= 2:
        ...
        score_details["factors"].append({
            "name": "topics",
            "value": 7,
            "reason": f"有 {topic_count} 个 topics"
        })
    elif topic_count >= 1:
        ...
        score_details["factors"].append({
            "name": "topics",
            "value": 4,
            "reason": f"有 {topic_count} 个 topic"
        })
    ...
    if server.get("license"):
        ...
        score_details["factors"].append({
            "name": "license",
            "value": 5,
            "reason": f"使用 {server['license']} 许可证"
        })
    ...
    examples_keywords = ["example", "usage", "install", "quick start", "demo", "tutorial", "example:"]
    ...
    if categories and len(categories) > 1:
        ...
        score_details["factors"].append({
            ...
            "value": 6,
            "reason": f"多维度分类 ({len(categories)} 个)"
        })
    ...
    verified_owners = {
        ...
        'bilibili', 'feishu', 'dingtalk', 'microsoft'
    }
    ...
    if len(platforms) >= 3:
        ...
        score_details["factors"].append({
            "name": "platforms",
            "value": 5,
            "reason": f"跨平台支持 ({', '.join(set(platforms))})"
        })
