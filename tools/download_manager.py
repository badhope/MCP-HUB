...
class MCPServerDownloader:
    ...
    def _generate_install_info(self, source: str, language: str, server_data: Dict) -> tuple:
        ...
        if 'github.com' in source:
            # 标准化 GitHub URL
            parts = source.replace('https://github.com/', '').replace('http://github.com/', '').split('/')
            if len(parts) >= 2:
                ...
                if language.lower() in ['javascript', 'typescript', 'node', 'nodejs']:
                    package_name = f'@modelcontextprotocol/server-{repo}' if '@' not in repo else repo
    ...
    def _generate_config(self, name: str, command: str, args: List[str], env: Dict[str, str]) -> Dict:
        """生成 MCP 配置文件"""
        return {
            "mcpServers": {
                name: {
...
"""
    ...
    def sync_from_index(self, index_path: Path, limit: Optional[int] = None):
        ...
        registry['downloaded'] = sum(1 for s in templates.values() if s['download_status'] == 'downloaded')
    ...
    def download_batch(self, server_names: List[str], parallel: int = 3) -> Dict[str, bool]:
        ...
        for name in server_names:
            ...
            template = MCPServerTemplate(**{k: v for k, v in template_data.items() if k != 'error_message'}, error_message=template_data.get('error_message'))
        ...
        registry['downloaded'] = sum(1 for s in registry['servers'].values() if s['download_status'] == 'downloaded')
        self.save_registry(registry)
