...
def main() -> None:
    """Generate the static-data bundle the frontend reads at runtime.

    Idempotent: safe to re-run. The whole pipeline used to live at module
    top level, which meant `import` would re-execute the file (and re-write
    JSON). Wrapping it here makes the module importable and `python3 -m
    tools.gen_static_data` / `python3 tools/gen_static_data.py` both work
    via the __main__ guard at the bottom.
    """
